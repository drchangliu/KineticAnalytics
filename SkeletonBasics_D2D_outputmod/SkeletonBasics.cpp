    ...
    m_pNuiSensor(NULL)
{
	...
	//Elaborate date and time filename setup
	//playback_output("playback_log_");
	//delimited_output("\t", "kinectlog_");
	//original_output("kinectlog_");
	//
	///////
	test=0;
	frame_count = 0;
...
void CSkeletonBasics::new_main_out(){
	...
	fenout << "Which is " << FINAL_ENERGY*0.239*0.001 << " Calories" << std::endl;
	fenout.close();
}
...
LRESULT CALLBACK CSkeletonBasics::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
		...
		if (IDC_BUTTON3 == LOWORD(wParam) && BN_CLICKED == HIWORD(wParam)){
			...
			for (int t = 1; t < COMSES.size(); t++){
				EnergySkeleton temp_e = EnergyPartials(COMSES[t - 1], COMSES[t]);
				FINAL_ENERGY+=Energy(temp_e);
				currentTotalEnergyExpended.push_back(FINAL_ENERGY);
			}
			new_main_out();
			coords.clear();
			COMSES.clear();
...
{
    int i;
	//fout << "At time " << time(NULL) <<  " ";
	//fout << "Begin frame " << frame_count << ":" << " ";
	std::vector<coord> tempvec;
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i)
    {
        m_Points[i] = SkeletonToScreen(skel.SkeletonPositions[i], windowWidth, windowHeight);
        coord temp;
		temp.x = skel.SkeletonPositions[i].x;
		temp.y = skel.SkeletonPositions[i].y;
		temp.z = skel.SkeletonPositions[i].z;
		if (tempvec.empty()) {
			tempvec.push_back(temp);
		}
		else {
			if ((abs(temp.x - tempvec.back().x))
				+ (abs(temp.y - tempvec.back().y))
				+ (abs(temp.z - tempvec.back().z)) < 2) {
				tempvec.push_back(temp);
			}
			else {
				tempvec.push_back(tempvec.back());
			}
		}
    }
	if (recording){
		coords.push_back(tempvec);
			COMSES.push_back(calculate_COM(skel));
			if (COMSES.size()>1){
				energyskels.push_back(EnergyPartials(COMSES[COMSES.size() - 2], COMSES[COMSES.size() - 1]));
			}
	}
	frame_count++;
